...
class TicTacToe {
    ...
    makeMove(move: MakeOptional<Move, 'player'>): void {
        ...
        if (!this.legalMoves.some(legalMove => legalMove.x === move.x && legalMove.y === move.y)) {
            throw new Error("Illegal move");
        }

        this.board.doMove({...move, player: this.currentPlayer});
        this.history.push({...move, player: this.currentPlayer});
        this.currentPlayer = this.currentPlayer === CellValue.X ? CellValue.O : CellValue.X;
    }

    get legalMoves(): Array<Move> {
        // For the first move, all moves are legal
        if (this.history.length === 0) {
            return Array(81).fill(0).map((_, i) => {
                return {
                    x: Math.floor(i / 9),
                    y: i % 9,
        ...
        if (this.board.getSuperCell(nextSubBoard.x, nextSubBoard.y) === CellValue.Empty) {
            // If the subboard has no winner, you must play in that subboard
            const moves: Array<Move> = [];
            for (let x = 0; x < 3; x++) {
                for (let y = 0; y < 3; y++) {
                    if (this.board.getCell(x + nextSubBoard.x * 3, y + nextSubBoard.y * 3) === CellValue.Empty) {
                        moves.push({
            ...
            for (let x = 0; x < 9; x++) {
                for (let y = 0; y < 9; y++) {
                    if (this.board.getCell(x, y) === CellValue.Empty && this.board.getSuperCell(Math.floor(x / 3), Math.floor(y / 3)) === CellValue.Empty) {
                        moves.push({
                            x,
                            y,
                            player: this.currentPlayer
